...
{
  auto ci = cindex();
  float corr = (float)fine / fine_if[device-192][ci] + fine_off[device-192][ci];
  if (fine > fine_cut[device-192][ci]) corr -= 1.;
...
lightdata::load_fine_calibration(std::string filename)
{
  ...
  for (int i = 0; i < 16; ++i) {
    ...
    for (int j = 0; j < 768; ++j) {
      ...
      fine_off[i][j] = hFine_off ? hFine_off->GetBinContent(j + 1) : -fine_if[i][j] * fine_cut[i][j];
    }
  }
  fin->Close();
  return true;
